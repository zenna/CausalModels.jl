...
#need float precision tolerance
allrowspvecs(K) = all(i->abs(sum(K[i,:])-1)<10^(-8),1:size(K)[1])
...
function convergencepower(K)
  n=1
  cur=K
  while !all(x->isapprox(x,0,atol=10^-8),cur*K-cur)
    cur=cur*K
    n+=1
  end
  return n
...
function simMetropolis1(rng, x, K, π)
  s=size(K)[1]
  J=[K[x,y]*min(1,π(y)/π(x)) for x in 1:s,y in 1:s]
  for i in 1:s
    J[i,i]+=1-sum(J[i,:])
  end
  println(J)
